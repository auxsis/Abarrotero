...
class AttendanceDevice(models.Model):
    ...
    device_name = fields.Char(string='Nombre del Dispositivo', readonly=True,
                               help="The model of the device which will be filled automatically when you hit the 'Get Device Info' button.")

    work_code = fields.Char(string='Codigo', readonly=True,
                               help="The Work Code of the device which will be filled automatically when you hit the 'Get Device Info' button.")

    ip = fields.Char(string="IP / Dominio", required=True, track_visibility='onchange', copy=False, readonly=True, states={'draft': [('readonly', False)]},
                     help='The accessible IP or Domain Name of the device to get device\'s attendance data', default='0.0.0.0')
    port = fields.Integer(string="Puerto", required=True, default=4370, track_visibility='onchange', readonly=True, states={'draft': [('readonly', False)]})
    ...
    device_users_count = fields.Integer(string='Numero de Usuarios', compute='_compute_device_users_count', store=True)

    mapped_employee_ids = fields.Many2many('hr.employee', 'mapped_device_employee_rel', string='Empleados Sincronizados',
                                           compute='_compute_employees', store=True,
                                           help="List of employees that have been mapped with this device's users")

    mapped_employees_count = fields.Integer(string='Numero de Empleados Sincronizados', compute='_compute_mapped_employees_count', store=True)

    umapped_device_user_ids = fields.One2many('attendance.device.user', 'device_id', string='Usuarios en Dispositivo',
    ...
    unmapped_employee_ids = fields.Many2many('hr.employee', 'device_employee_rel', 'device_id', 'employee_id',
                                             compute='_compute_employees', store=True, string='Empleados no Sincronizados',
                                             help="The employees that have not been mapped with any user of this device")

    attendance_device_state_line_ids = fields.One2many('attendance.device.state.line', 'device_id', string='Codigos de Estado', copy=False,
                                                       default=_get_default_attendance_device_state_lines,
                                                       readonly=True, states={'draft': [('readonly', False)]})
    location_id = fields.Many2one('attendance.device.location', string='Localizacion', track_visibility='onchange',
    ...
    auto_clear_attendance_schedule = fields.Selection([
        ...
        ('time_scheduled', 'Time Scheduled')], string='Auto Clear Programacion', required=True, default='on_download_complete', track_visibility='onchange',
    ...
    auto_clear_attendance_hour = fields.Float(string='Auto Clear a :', track_visibility='onchange', required=True, default=0.0,
    ...
    auto_clear_attendance_dow = fields.Selection([
        ...
        (4, 'Viernes'),
        (5, 'Sabado'),
        (6, 'Domingo'), ], string='Auto Clear On', default=-1, required=True, track_visibility='onchange')
    ...
    download_error_notification = fields.Boolean(string='Descargar Notificaciones de Error', default=True,
                                                 readonly=True, states={'draft': [('readonly', False)]},
    ...
    total_finger_template_records = fields.Integer(string='Huellas Digitales', compute='_compute_total_finger_template_records')
    protocol = fields.Selection([('udp', 'UDP'), ('tcp', 'TCP')], string='Protocolo', required=True, default='tcp',
                                track_visibility='onchange',
                                help="Algunos dispositivos anteriores no soportan TCP. en estos casos, intente cambiando a UDP.")
    omit_ping = fields.Boolean(string='Omitir Ping', default=True, help='Omit ping ip address when connecting to device.',
                               readonly=True, states={'draft': [('readonly', False)]})
