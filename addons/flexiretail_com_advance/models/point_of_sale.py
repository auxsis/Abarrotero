...
class PosConfig(models.Model):
    ...
    @api.onchange('multi_store_id')
    def on_change_multi_store_id(self):
        if self.multi_store_id:
            self.stock_location_id = self.multi_store_id.location_id.id

    @api.multi
    def write(self, vals):
        if vals.get('module_pos_restaurant'):
            raise Warning(_("You Can't Use Restaurant While Using FlexiRetail!"))
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        ...
        if user_rec and erp_manager_id not in user_rec.groups_id.ids:
            if user_rec.store_ids:
                args += ['|', ('multi_store_id', 'in', user_rec.store_ids.ids), ('multi_store_id', '=', False)]
    ...
    # POS Multi-Shop
    multi_store_id = fields.Many2one("pos.store", string="Shop")
    ...
    pos_crdt_journal_id = fields.Many2one('account.journal', string='Select Journal')
...
class pos_order(models.Model):
    ...
    def check_order_delivery_type(self):
        if self.delivery_type == 'pending' and self.state == 'draft':
            action_id = self.env.ref('flexiretail_com_advance.action_pos_payment_flexiretail')
            return {
                'name': action_id.name,
                'type': action_id.type,
...
class pos_session(models.Model):
    ...
    store_id = fields.Many2one('pos.store', string='Shop', related='config_id.multi_store_id')
    ...
    def get_total_first(self):
        ...
        if self:
            total = ((
                                 self.get_total_sales()[1] + self.get_total_tax()[1] + self.get_money_in_total()[1] + self.cash_register_balance_start) + self.get_money_out_total()[1]) \
