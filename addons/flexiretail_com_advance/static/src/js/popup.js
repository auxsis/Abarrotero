odoo.define('flexiretail_com_advance.popup', function (require) {
    "use strict";
    ...
    var CreateProductPopupWidget = PopupWidget.extend({
        ...
        save_product_details: function() {
            ...
            if (!fields.name) {
                ...
                self.pos.db.notification('danger',_t('A Product UOM Is Required'));
                return;
            }else {
            ...
            this.keypress_order_number = function(e){
                if(e.which === 13){
                    ...
                    if(self.pos.config.multi_store_id && self.pos.config.multi_store_id[0]){
                        domain = ['|',['store_id','=',self.pos.config.multi_store_id[0]],['store_id','=',false],['pos_reference','=', ret_o_ref_order]];
                    } else{
...
//	    		self.gui.show_screen('products');
    ...
    var RedeemLoyaltyPointsPopup = PopupWidget.extend({
        ...
        close: function(){
            $('body').off('keypress', self.keyboard_handler).keypress(this.payment_self.keyboard_handler);
            $('body').off('keydown',self.keyboard_keydown_handler).keydown(this.payment_self.keyboard_keydown_handler);
    ...
    var PaymentSummaryReportPopupWizard = PopupWidget.extend({
        ...
        show: function(options){
            ...
            if(this.pos.config.current_month_date){
                $('input#pay_summary_start_date').val(first_date_of_month);
                $('input#pay_summary_end_date').val(today_date);
            }
            $("#pay_summary_start_date").change(function() {
                if($("#pay_summary_start_date").val()){
                     $('#pay_summary_start_date').css('border','');
                }
            });
            $("#pay_summary_end_date").change(function() {
                if($("#pay_summary_end_date").val()){
                    $('#pay_summary_end_date').css('border','');
                }
            });
            $('input#pay_summary_start_date').focus();
        },
        click_confirm: function(){
            var self = this;
            var order = this.pos.get_order();
            var from_date = this.$('input#pay_summary_start_date').val();
            var to_date = this.$('input#pay_summary_end_date').val();
            var today_date = new Date().toISOString().split('T')[0];
            ...
            if(from_date == "" && to_date == "" || from_date != "" && to_date == "" || from_date == "" && to_date != "" ){
                if(!from_date){
                    $('#pay_summary_start_date').css('border','1px solid red');
                }
                if(!to_date){
                    $('#pay_summary_end_date').css('border','1px solid red');
                }
                return;
...
//    Product Summary Report
    var ProductSummaryReportPopupWizard = PopupWidget.extend({
        ...
        show: function(options){
            ...
            $('input#start_date_prod_summary').focus();
            ...
            if(this.pos.config.product_summary_month_date){
                $('input#start_date_prod_summary').val(first_date_of_month);
                $('input#end_date_prod_summary').val(today_date);
            }
            $("#start_date_prod_summary").change(function() {
                if($("#start_date_prod_summary").val() != ""){
                     $('#start_date_prod_summary').css('border','');
                }
            });
            $("#end_date_prod_summary").change(function() {
                if($("#end_date_prod_summary").val() != ""){
                    $('#end_date_prod_summary').css('border','');
                }
            });
            if(no_of_report <= 0){
        ...
        click_confirm: function(){
            var self = this;
            var from_date = $('input#start_date_prod_summary').val();
            var to_date = $('input#end_date_prod_summary').val();
            ...
            if(from_date == "" && to_date == "" || from_date != "" && to_date == "" || from_date == "" && to_date != "" ){
                if(from_date == ""){
                    $('#start_date_prod_summary').css('border','1px solid red');
                }
                if(to_date == ""){
                    $('#end_date_prod_summary').css('border','1px solid red');
                }
                return;
            } else if(from_date > to_date){
                alert("Start date should not be greater than end date");
    ...
    var OrderSummaryPopupWidget = PopupWidget.extend({
        ...
        show: function(options){
            ...
            if(this.pos.config.order_summary_current_month){
                $('input#start_order_date').val(first_date);
                $('input#end_order_date').val(today_date);
            }
        },
        click_confirm: function(){
            var self = this;
            ...
            self.pos.from_date = this.$('input#start_order_date').val();
            self.pos.to_date = this.$('input#end_order_date').val();
            var today_date = new Date().toISOString().split('T')[0];
            ...
            if(self.pos.from_date == "" && self.pos.to_date == "" || self.pos.from_date != "" && self.pos.to_date == "" || self.pos.from_date == "" && self.pos.to_date != "" ){
                if(self.pos.from_date == ""){
                    $('#start_order_date').css('border','1px solid red');
                }
                if(self.pos.to_date == ""){
                    $('#end_order_date').css('border','1px solid red');
                }
                return;
    ...
    var PackLotLinePopupWidget = PopupWidget.extend({
        ...
        render_list: function(orders){
            ...
            for(var i = 0, len = Math.min(orders.length,1000); i < len; i++){
                var serial    = orders[i];
                serial.check_expire_alert =moment(new moment().add(self.pos.config.product_exp_days, 'd').locale('en').format('YYYY-MM-DD HH:mm:mm')).locale('en').format('YYYY/MM/DD');
...
//	Multi-Store Popup
        ...
        click_confirm: function(){
            ...
            if(self.pos.store_rec){
                var params = {
                    model: 'pos.session',
                    method: 'write',
                    args: [self.pos.pos_session.id,{'store_id':Number(self.selected_id)}],
                }
                if(self.selected_id){
                    rpc.query(params, {async: false}).then(function(result){
                    if(result){
                ...
                var param_config = {
                    model: 'pos.config',
                    method: 'write',
                    args: [self.pos.config.id,{'stock_location_id':self.pos.store_rec.location_id[0],'multi_store_id':self.pos.store_rec.id}],
                }
                if(self.selected_id){
                    if(self.selected_id == self.pos.pos_session.store_id[0]){
                        return;
                    }
                    else{
    ...
    var ReportPopupWidget = PopupWidget.extend({
        ...
        session_report_pdf: function(e){
            ...
            self.pos.chrome.do_action('flexiretail_com_advance.report_pos_inventory_session_pdf_front',{additional_context:{
        ...
        session_report_thermal: function(e){
            var self = this;
            var session_id = $(e.currentTarget).data('id');
            var report_name = "flexiretail_com_advance.front_inventory_session_thermal_report_template";
            var params = {
                model: 'ir.actions.report',
                method: 'get_html_report',
        ...
        location_report_pdf: function(e){
            ...
            self.pos.chrome.do_action('flexiretail_com_advance.report_pos_inventory_location_pdf_front',{additional_context:{
        ...
        location_report_thermal: function(e){
            var self = this;
            var location_id = $(e.currentTarget).data('id');
            var report_name = "flexiretail_com_advance.front_inventory_location_thermal_report_template";
            var params = {
                model: 'ir.actions.report',
                method: 'get_html_report',
...
//	        $('#orderdate-datepicker').focus();
        ...
        click_confirm: function(){
            var self = this;
            if($('#orderdate-datepicker').val() >= $('#requesteddate-datepicker').val()){
                self.pos.db.notification('danger',"Reqested date should be greater then start date!");
                return
            }
            var order = self.pos.get_order();
...
//	        var valid_start_date;
            $('#orderdate-datepicker').datetimepicker({
                    format:'Y-m-d H:i',
//					 onChangeDateTime:function(dp,$input){
//						 valid_start_date = $input.val();
//					 }
            ...
            $('#requesteddate-datepicker').datetimepicker({
                format:'Y-m-d H:i',
//				onChangeDateTime:function(dp,$input){
//				 	if(valid_start_date > $input.val()){
//				 		self.pos.db.notification('danger',"Connection lost");
//				 		return
//				 	}
//				 },
                minDate: new Date($('#orderdate-datepicker').val()),
...
//    Credit Debit Stmt Popup
    var PrintCreditDebitStatmentPopup = PopupWidget.extend({
        template: 'PrintCreditDebitStatmentPopup',
        show: function(options){
            var self = this;
            self._super(options);
        ...
        click_confirm: function(){
            var self = this;
            var order = self.pos.get_order();
            var start_date = $('#credit_stmt_st_date').val();
            var end_date = $('#credit_stmt_ed_date').val();
    ...
    gui.define_popup({name:'credit_debit_statement_popup', widget: PrintCreditDebitStatmentPopup});

    var AddMoneyToCreditPopup = PopupWidget.extend({
        template: 'AddMoneyToCreditPopup',
        events: _.extend({}, PopupWidget.prototype.events, {
            'keypress #amount-to-be-added': 'keypress_amount_to_be_added',
         }),
         keypress_amount_to_be_added: function(event){
         ...
         click_confirm: function(){
             ...
             this.pay_due = $(".pay_due_amount_credit_debit").prop('checked');
             var params = {
        ...
        renderElement: function() {
            var self = this;
            self._super();
            $('.pay_due_amount_credit_debit').click(function(){
                if (!$(this).is(':checked')) {
        ...
        click_confirm: function(){
            ...
            if(journal_id != 0){
                if(self.order_id){
                    ...
                    .then(function(result){
                        if(result){
                            var exist_order = _.findWhere(self.pos.get('pos_order_list'), {'pos_reference': result.pos_reference})
                            if(exist_order){
                                _.extend(exist_order, result);
                            } else {
                                self.pos.get('pos_order_list').push(result);
                            }
                            var new_orders = _.sortBy(self.pos.get('pos_order_list'), 'id').reverse();
                            self.pos.db.add_orders(new_orders);
                            self.pos.set({ 'pos_order_list' : new_orders });
