odoo.define('flexiretail_com_advance.models', function (require) {
	"use strict";
	...
	models.load_fields("res.users", ['image_small','login','company_ids','access_ereceipt','access_quick_cash_payment',
	                                 ...
	                                 'access_product_expiry_report','pos_user_type','sales_persons','display_own_sales_order',
	                                 'access_print_ledger','access_default_customer','rfid_no']);
	...
	models.load_fields('pos.session',['cash_register_id','is_lock_screen','current_cashier_id','locked','locked_by_user_id','opening_balance','store_id']);
	models.load_fields("account.journal", ['shortcut_key','jr_use_for','apply_charges','fees_amount','fees_type','optional']);
	        	...
	        	if(user && user.pos_user_type == "salesman"){
	                ...
	                if(user && user.sales_persons && user.sales_persons.length > 0){
	                   ...
	                   if(user_ids && user_ids.length > 0){
    	                   domain_order.push(['salesman_id', 'in', user_ids]);
	                   } else {
    	                   domain_order.push(['salesman_id','=',false]);
	                   }
	                } else {
	                    domain_order.push(['salesman_id', '=', user.id]);
	                }
...
//	    			domain_order.push('|',['store_id','=',false],['store_id','=',self.config.multi_store_id[0]],['state','not in',['cancel']], ['create_date', '>=', from_date]);
	    			domain_order.push(['store_id','=',self.config.multi_store_id[0]],['state','not in',['cancel']], ['create_date', '>=', from_date]);
				} else{
            ...
            var records = rpc.query({
                ...
                if(orders && orders[0]){
                            ...
                            if(order_count_loop > 0){
                                ...
                                $.ajax({
                                    ...
                                    success: function(res) {
                                        ...
                                        self.db.add_orders(pos_all_orders);
                                        if(self.get('pos_order_list') && self.get('pos_order_list').length > 0){
                                            _.each(pos_all_orders, function(each_order) {
                                                 self.get('pos_order_list').push(each_order);
                                            });
                                        } else{
                                            self.set({'pos_order_list' : pos_all_orders});
                                        }
                                        if(self.config.orders_sync && self.config.enable_reorder){
                                            self.chrome.render_sale_note_order_list(pos_all_orders);
                                        }
                                        if(remaining_time < 100){
                                            self.load_order_flag = false;
                                        } else{
                                            self.load_order_flag = true;
                                        }
                                        ajax_all_pos_order_load();
                                    },
                                    error: function(e) {
                                        console.log("e >>>>>>>> ",e);
                                        self.db.notification('danger',_t('Loading POS Order Failed !'));
                    ...
                    console.log("\n\n Not Any Order Found.");
                    self.db.notification('danger',_t(' Not Any Order Found !'));
                }
            }).fail(function(){
                self.db.notification('danger',"Connection lost");
            });
			...
			/*if(self.config.orders_sync && self.config.enable_reorder){
	        	...
	        	var domain_order = [];
	        	var user_ids = [];
	        	if(user.pos_user_type=="salesman"){
	            	domain_order.push(['salesman_id', '=', user.id]);
	            } else if(user.pos_user_type=="cashier") {
	                if(user.sales_persons && user.sales_persons.length > 0){
	                   var selected_users = user.sales_persons;
	                   _.each(selected_users, function(each_user_id){
	                        user_ids.push(each_user_id)
	                   });
				...
				if(self.config.multi_store_id && self.config.multi_store_id[0]){
					self.domain_as_args = ['|',['store_id','=',false],['store_id','=',self.config.multi_store_id[0]],['state','not in',['cancel']], ['create_date', '>=', from_date]];
				} else{
...
//				this.orderlines.on('change', this.pos.send_new_notif, this.pos);
//				// removing last orderline does not trigger change event
//				this.orderlines.on('remove',   this.pos.send_new_notif, this.pos);
//				this.paymentlines.on('change', this.pos.send_new_notif, this.pos);
//				// removing last paymentline does not trigger change event
//				this.paymentlines.on('remove', this.pos.send_new_notif, this.pos);
//			}
            this.set({
            	paying_order: false,
...
//            		  var change = this.get_total_paid() - this.getNetTotalTaxIncluded() - this.get_order_total_discount();
        ...
        get_change_amount_for_wallet: function() {
            return this.get('change_amount_for_wallet') ? Number(this.get('change_amount_for_wallet')) : false;
        },
...
// Credit debit finish here.
	    ...
	    export_as_JSON: function() {
            ...
            var new_val = {
                ...
                store_id : self.pos.config.multi_store_id ? self.pos.config.multi_store_id[0] : false,
