odoo.define('flexiretail_com_advance.screens', function (require) {
	"use strict";
	...
	var flexi_chrome = require('flexiretail_com_advance.chrome')
...
//            			self.pos.chrome.screens.products.render_product_category([categ]);
            			self.pos.chrome.screens.products.set_back_to_parent_categ(categ.parent_id[0]);
            		}else{
            		...
            		$(".v-category[data-category-id='"+self.start_categ_id+"']").trigger('click');
            	}, 2000);
            }
...
//    						                    domain: JSON.stringify(self.pos.product_domain),
    						            ...
    						            success: function(res) {
    					            		...
    					            		if(count_loaded_products >= total_products){
    					            			...
    					            			$('.product_progress_bar').delay(3000).fadeOut('slow');
    					            		} else if(count_loaded_products == total_products)  {
...
//                                    self.click_paymentmethods_by_journal(self.pos.config.pos_crdt_journal_id[0]);
...
//            var customer_display = this.pos.config.customer_display;
            ...
            if(self.pos.config.enable_card_charges && self.pos.get_cashier().access_card_charges) {
		        ...
		        lines.on('focusout','.payment_charge_input',function(){
		        	$('body').off('keypress', self.keyboard_handler).on('keypress', self.keyboard_handler);
	            	$('body').off('keydown',self.keyboard_keydown_handler).on('keydown',self.keyboard_keydown_handler);
		        	/*window.document.body.addEventListener('keypress',self.keyboard_handler);
	                window.document.body.addEventListener('keydown',self.keyboard_keydown_handler);*/
		        });
	        }
...
//    			var order = self.pos.get_order();
//    			if(self.pos.config.multi_store){
//	    			if(self.pos.pos_session.store_id && self.pos.pos_session.store_id[0]){
//	                    var store_rec = self.pos.store_by_id[self.pos.pos_session.store_id[0]];
//	                    if(self.pos.get_cashier().id == store_rec.store_manager[0]){
//	                        var order = self.pos.get_order();
...
//	                        }
//	                    } else{
//	                        alert("You have no access for this store!")
//	                    }
//	                }else{
    ...
    screens.ProductScreenWidget.include({
    	start: function(){
	    	...
	    	$('#multi-shop').click(function(){
	            var user_stores = self.pos.get_cashier().store_ids;
	            var store_list = [];
...
//        	$('.breadcrumb-button.breadcrumb-home.js-category-switch').trigger('click')
	...
	var OrderListScreenWidget = screens.ScreenWidget.extend({
	    ...
	    init: function(parent, options){
            ...
            /*var domain_order = [];
            if(self.pos.config.orders_sync && self.pos.config.enable_reorder){
				var from_date = moment().locale('en').format('YYYY-MM-DD');
				if(self.pos.config.last_days){
					from_date = moment().subtract(self.pos.config.last_days, 'days').locale('en').format('YYYY-MM-DD');
				}
	        	var user_ids = [];
	        	var user = self.pos.get_cashier() || false;
	        	if(user && user.pos_user_type == "salesman"){
	            	domain_order.push(['salesman_id', '=', user.id]);
	            } else if(user && user.pos_user_type == "cashier") {
	                if(user && user.sales_persons && user.sales_persons.length > 0){
	                   var selected_users = user.sales_persons;
	    		...
	    		if(self.pos.config.multi_store_id && self.pos.config.multi_store_id[0]){
	    			domain_order.push('|',['store_id','=',false],['store_id','=',self.pos.config.multi_store_id[0]],['state','not in',['cancel']], ['create_date', '>=', from_date]);
				} else{
					domain_order.push(['state','not in',['cancel']], ['create_date', '>=', from_date]);
				}
			} else if((self.pos.config.enable_reorder && !self.pos.config.orders_sync) || self.pos.config.enable_credit){
                var from_date = moment().locale('en').format('YYYY-MM-DD')
                if(self.pos.config.last_days){
                    from_date = moment().subtract(self.pos.config.last_days, 'days').locale('en').locale('en').format('YYYY-MM-DD');
                }
                if(self.pos.config.multi_store_id && self.pos.config.multi_store_id[0]){
                    domain_order.push('|',['store_id','=',false],['store_id','=',self.pos.config.multi_store_id[0]],['state','not in',['cancel']], ['create_date', '>=', from_date]);
                } else{
		    ...
		    console.log("\n\n domain_as_args=====>",domain_order)
            var records = rpc.query({
                model: 'pos.order',
                method: 'calculate_order',
                args: [{'domain': domain_order}],
            }, {async: false}).then(function (orders) {
                if(orders && orders[0]){
                    var all_order_ids = _.uniq(orders);
                    var total_order_ids = all_order_ids.length;
                    var remaining_time;
                    if(total_order_ids){
                        var order_limit = 1000;
                        var order_count_loop = all_order_ids.length;
                        var order_last_ids = all_order_ids;
                        var count_loaded_orders = 0;
                        function ajax_all_pos_order_load(){
                            if(order_count_loop > 0){
                                console.log("\n\n order count --->",order_count_loop)
                                $.ajax({
                                    type: "POST",
                                    url: '/web/dataset/load_all_pos_order',
                                    data: {
                                            model: 'pos.order',
                                            order_limit:order_limit,
                                            result_order_ids:JSON.stringify(order_last_ids.splice(0, order_limit) || []),
                                    },
                                    success: function(res) {
                                        var pos_all_orders = JSON.parse(res);
                                        order_count_loop -= pos_all_orders.length;
                                        remaining_time = ((total_order_ids - order_count_loop) / total_order_ids) * 100;
                                        self.pos.db.add_orders(pos_all_orders);
                                        if(self.pos.get('pos_order_list') && self.pos.get('pos_order_list').length > 0){
                                            _.each(pos_all_orders, function(each_order) {
                                                 self.pos.get('pos_order_list').push(each_order);
                                            });
                                        } else{
                                            self.pos.set({'pos_order_list' : pos_all_orders});
                                        }
                                        if(self.pos.config.orders_sync && self.pos.config.enable_reorder){
                                            self.pos.chrome.render_sale_note_order_list(pos_all_orders);
                                        }
                                        if(remaining_time < 100){
                                            $('#order_screen_freeze_screen').addClass("active_order_freeze_screen");
                                            $('span.order_screen_freeze').show();
                                        } else{
                                            $('#order_screen_freeze_screen').removeClass("active_order_freeze_screen");
                                            $('span.order_screen_freeze').hide();
                                        }
                                        ajax_all_pos_order_load();
                                    },
                                    error: function(e) {
                                        console.log("e >>>>>>>> ",e);
                                        self.pos.db.notification('danger',_t('Loading POS Order Failed !'));
                                        console.log('No Any Order Found !');
                                    },
                                });
                            }
                        }
                    ...
                    console.log("\n\n Not Any Order Found.");
                    self.pos.db.notification('danger',_t(' Not Any Order Found !'));
                }
            }).fail(function(){
                self.pos.db.notification('danger',"Connection lost");
            });*/
...
//	        'click #view_lines': 'click_view_lines',
		...
		start: function(){
	    	var self = this;
	    	this._super();
	    	$('span.order_datefilter input#datepicker').datepicker({
           	    dateFormat: 'yy-mm-dd',
                autoclose: true,
                closeText: 'Clear',
                        	...
                        	if(results){
                                 _.each(results, function(res) {
                                     ...
                                     if(product){
                                         ...
                                         if(res.pack_lot_ids && res.pack_lot_ids.length > 0){
                                        	var pack_lot_lines = self.get_pack_lot_line_for_line(res.pack_lot_ids);
                                        	line.set_input_lot_serial(pack_lot_lines);
                                         }
                                     }
                                 });
	    ...
	    get_pack_lot_line_for_line: function(pack_lot_ids){
	    	var pack_lot_lines = []
	    	var params = {
	    	...
	    	return pack_lot_lines
	    },
        show: function(){
	        this._super();
	        this.reload_orders();
	        $('.button.paid').removeClass('selected').trigger('click');
	        if(this.pos.load_order_flag){
	            $('#order_screen_freeze_screen').removeClass("active_order_freeze_screen");
                $('#order_screen_freeze_screen').hide();
	        } else {
                $('#order_screen_freeze_screen').show();
                $('#order_screen_freeze_screen').addClass("active_order_freeze_screen");
	        }
	    },
    ...
    var GiftVoucherListScreenWidget = screens.ScreenWidget.extend({
        ...
        init: function(parent, options){
            ...
            this.reload_btn = function(){
                ...
                self.$el.find('.voucherbtn input').val('');
        ...
        start: function(){
            ...
            /*$('.search_voucher_expiry_date').datepicker({
                dateFormat: 'yy-mm-dd',
                autoclose: true,
                closeText: 'Clear',
                showButtonPanel: true,
...
//            			self.gui.show_popup('show_product_popup',{'product':product});
        ...
        click_receipt: function(){
            ...
            if(list_product.length > 0){
                order.set_product_vals(self.stock_products)
...
//                }
        ...
        /*render_products: function(products){
            ...
            console.log("stock product-->",stock_products)
        ...
        render_products: function(products){
            ...
            for(var i = 0, len = products.length; i < len; i++){
                ...
                if(product){
                	if(location_id){
                        if(!product.is_dummy_product && product.type == 'product'){
                            self.stock_products.push(product)
                        }
                    }else{
                        if(!product.is_dummy_product && product.type == 'product' && product.qty_available <= 0){
                            self.stock_products.push(product)
...
//        	$('.manage_list_view').html(QWeb.render('OutStockListView',{widget: self,products: self.all_products}));
	...
	var SaleOrderListScreenWidget = screens.ScreenWidget.extend({
	    ...
	    start: function(){
            ...
            this.render_list(orders);
            $('input#datepicker_sale_order').datepicker({
           	    dateFormat: 'yy-mm-dd',
                autoclose: true,
                closeText: 'Clear',
                showButtonPanel: true,
	...
	var InvoiceListScreenWidget = screens.ScreenWidget.extend({
	    ...
	    renderElement: function(){
	    	var self = this;
	    	self._super();
	    	self.el.querySelector('.button.reload.invoice_btn').addEventListener('click',this.reload_btn);
	    },
	});
...
*********************************************************** */
	var SelectVariantScreen = screens.ScreenWidget.extend({
        ...
        start: function(){
			...
			this.variant_list_widget = new flexi_chrome.VariantListWidget(this,{});
			...
			this.attribute_list_widget = new flexi_chrome.AttributeListWidget(this,{});
    ...
    return {
    	...
    	ProductsScreenWidget:ProductsScreenWidget,
    	SaleOrderListScreenWidget:SaleOrderListScreenWidget,
    	InvoiceListScreenWidget:InvoiceListScreenWidget,
    	CustomerCreditListScreenWidget:CustomerCreditListScreenWidget,
    	OrderListScreenWidget:OrderListScreenWidget,
    	GraphScreenWidget:GraphScreenWidget,
    	POSDashboardGraphScreenWidget:POSDashboardGraphScreenWidget,
    	ProductExpiryDeshboardWidget:ProductExpiryDeshboardWidget,
    	ProductDetailListScreen:ProductDetailListScreen,
    	OutStockProductsScreenWidget:OutStockProductsScreenWidget,
    	OrderDetailScreenWidget:OrderDetailScreenWidget,
    	GiftCardListScreenWidget:GiftCardListScreenWidget,
    	GiftVoucherListScreenWidget:GiftVoucherListScreenWidget,
    	DiscardProductScreenWidget:DiscardProductScreenWidget,
